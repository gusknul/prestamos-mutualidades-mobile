...
public class MainActivity extends Activity {
	...
	protected void onResume() {
		...
		if (adapterSocio.obtenerTotalCobros() < 0
				&& adapterSocio.obtenerTotalPagos() < 0) {
			Toast.makeText(this,
					"No hay base de datos, Actulice su base de datos",
					Toast.LENGTH_SHORT).show();
		} else {
			pagosTotales.setText(" Total Pagos: $"
					+ String.valueOf(adapterSocio.obtenerTotalPagos()));
			cobrosTotales.setText(" Total Cobranza: $"
					+ String.valueOf(adapterSocio.obtenerTotalCobros()));
		}
	...
	public void sendDataToServer(View view) {
		...
		if (!ip.getText().toString().equals("")) {
					...
					new AsyncHttpResponseHandler() {
						...
						public void onSuccess(String response) {
							...
							try {
								...
								boolean success = respuesta
										.getBoolean("success");
								if (success) {
									Toast.makeText(MainActivity.this,
											"Base de datos enviada con exito",
			...
			Toast.makeText(MainActivity.this,
					"Ingrese una direccion ip valida", Toast.LENGTH_SHORT)
					.show();
		}
